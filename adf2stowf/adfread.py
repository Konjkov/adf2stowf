...
def adfread(infname='TAPE21.asc', outfile=None):
    ...
    try:
        ...
        while i < len(lines):
            ...
            key = lines[i].strip()
            i += 1
            len1, len2, typ = [int(s) for s in lines[i][:-1].split()]
            i += 1
            # assert len1 == len2
            ...
            if typ == 1:  # integer
                ...
                while len(value) < len2:
                    value += [intX(s) for s in splitN(lines[i][:-1], 12)]
                    i += 1
                ...
                value = []
                while len(value) < len2:
                    value += [floatX(s) for s in splitN(lines[i][:-1], 28)]
                    i += 1
                assert len(value) == len2
                ...
                value = []
                while len(value) < len2:
                    value += [{'T': True, 'F': False}[s] for s in lines[i][:-1]]
                    i += 1
                assert len(value) == len2
            else:
                raise ValueError('type 1..4 expected')
